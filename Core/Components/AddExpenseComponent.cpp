...
void AddExpenseComponent::OnUIUpdate()
{
    ImGuiDraw("Aggiungi una nuova spesa", [] {
        ...
        std::time_t time = std::chrono::system_clock::to_time_t(nowDate);
        std::tm *dateSplit = std::localtime(&time);
        
        static int Day = dateSplit->tm_mday, Month = dateSplit->tm_mon + 1, Year = dateSplit->tm_year + 1900;
        ...
        bool ThingsBoughtEmpty = strlen(ThingsBought) == 0;
